<!doctype html>
<html class="no-js" lang="zxx">


<!-- Mirrored from html.themewin.com/edurock-preview/edurock/course-details-2.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 25 Apr 2024 06:17:09 GMT -->
<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title> ষ্টুডিও আর্ট : ১৬+ বছর - ফ্রগবিড একাডেমি</title>
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <?php include ('include/css.php');?>

</head>


<body class="body__wrapper">
<!-- pre loader area start -->
<?php include ('include/preloader.php');?>
<!-- pre loader area end -->

<!-- Dark/Light area start -->
<?php include ('include/dark_light.php');?>
<!-- Dark/Light area end -->

<main class="main_wrapper overflow-hidden">

    <!-- topbar__section__stert -->
    <?php include ('include/topbar.php');?>
    <!-- topbar__section__end -->

    <!-- headar section start -->
    <?php include ('include/header.php');?>
    <!-- header section end -->

    <!-- Mobile Menu Start Here -->
    <?php include ('include/mobile_menu.php');?>
    <!-- Mobile Menu end Here -->

    <!-- theme fixed shadow -->
    <div>
        <div class="theme__shadow__circle"></div>
        <div class="theme__shadow__circle shadow__right"></div>
    </div>
    <!-- theme fixed shadow -->
    <!-- breadcrumbarea__section__start -->
    <div class="breadcrumbarea breadcrumbarea--2">

        <div class="container">
            <div class="row">
                <div class="col-xl-8">
                    <div class="breadcrumb__content__wraper" data-aos="fade-up">
                        <div class="breadcrumb__inner text-start">
                            <ul>
                                <li><a href="index-2.html"> হোম </a></li>
                                <li> ষ্টুডিও আর্ট </li>
                            </ul>
                        </div>
                    </div>

                    <div class="course__details__top--2">
                        <div class="course__details__heading" data-aos="fade-up">
                            <h3> ষ্টুডিও আর্ট (১৬+ বছর বয়সীদের জন্য )</h3>
                        </div>
                        <div class="course__details__price" data-aos="fade-up">
                            <ul>
                                <li>
                                    <div class="course__details__date">
                                        <i class="icofont-book-alt"></i> সপ্তাহে ২ টি ক্লাশ
                                    </div>

                                </li>

                                <li>
                                    <div class="course__date">
                                        <i class="icofont-clock-time"></i> প্রতিটি ক্লাশ <span> ০১ঃ৩০ মিনিট </span>
                                    </div>
                                </li>
                            </ul>
                        </div>
                    </div>

                </div>
            </div>
        </div>

        <div class="shape__icon__2">
            <img loading="lazy"  class=" shape__icon__img shape__icon__img__1" src="img/herobanner/herobanner__1.png" alt="photo">
            <img loading="lazy"  class=" shape__icon__img shape__icon__img__2" src="img/herobanner/herobanner__2.png" alt="photo">
            <img loading="lazy"  class=" shape__icon__img shape__icon__img__3" src="img/herobanner/herobanner__3.png" alt="photo">
            <img loading="lazy"  class=" shape__icon__img shape__icon__img__4" src="img/herobanner/herobanner__5.png" alt="photo">
        </div>

    </div>
    <!-- breadcrumbarea__section__end-->

    <div class="blogarea__2 sp_top_50 sp_bottom_100">
        <div class="container">
            <div class="row">
                <div class="col-xl-8 col-lg-8">
                    <div class="blog__details__content__wraper">
                        <div class="course__details__tab__wrapper" data-aos="fade-up">
                            <div class="row">
                                <div class="col-xl-12">
                                    <ul class="nav  course__tap__wrap" id="myTab" role="tablist">
                                        <li class="nav-item" role="presentation">
                                            <button class="single__tab__link active" data-bs-toggle="tab" data-bs-target="#projects__one" type="button"><i class="icofont-paper"></i> কারিকুলাম </button>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div class="tab-content tab__content__wrapper active show" id="myTabContent">
                                <div class="tab-pane fade active show" id="projects__two" role="tabpanel" aria-labelledby="projects__one">
                                    <div class="experence__heading">
                                        <h5> আমাদের কোর্সটিতে যা থাকছে </h5>
                                    </div>
                                    <div class="experence__description">
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>ব্যাসিক টু  অ্যাডভান্স ড্রয়িং</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>ব্যাসিক ডিজাইন থিউরি</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>অ্যাডভান্স ওয়াটার কালার</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>আক্র‍্যালিক  পেইন্টিং</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>কালার মিক্সিং এন্ড ট্রিটমেন্ট</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>ষ্টীল লাইফ- স্কেচ,ওয়াটার কালার, পেইন্টিং</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>আউটডোর ল্যান্ডস্ক্যাপ-স্কেচ, ওয়াটার কালার</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>পোর্ট্রেট এন্ড ফিগার ড্রয়িং বেসিক এন্ড স্কেচ</h5>
                                            </div>
                                        </div>
                                        <div class="scc__wrap">
                                            <div class="scc__info">
                                                <h5>কালার থিউরি এন্ড কম্পোজিশন অয়েল  পেইন্টিং</h5>
                                            </div>
                                        </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4">
                    <div class="course__details__sidebar--2">
                        <div class="event__sidebar__wraper" data-aos="fade-up">
                            <div class="blogarae__img__2 course__details__img__2" data-aos="fade-up">
                                <img loading="lazy"  src="img/blog/blog_7.png" alt="blog">
                            </div>

                            <div class="event__price__wraper">

                                <div class="event__price">
                                    ৳১,২০০ <del>/ ৳১,৫০০</del>
                                </div>
                                <div class="event__Price__button">
                                    <a href="#">২০% ছাড়</a>
                                </div>
                            </div>

                            <div class="course__summery__lists">
                                <ul>
                                    <li>
                                        <div class="course__summery__item">
                                            <span class="sb_label"> ইন্সট্রাকটর:</span><span class="sb_content"><a
                                                    href="instructor-details.html"> বিপ্লব কুমার মন্ডল </a></span>
                                        </div>
                                    </li>

                                    <li>
                                        <div class="course__summery__item">
                                            <span class="sb_label">কোর্স লেভেল</span><span
                                                class="sb_content"> ইন্টারমিডিয়েট </span>
                                        </div>
                                    </li>

                                    <li>
                                        <div class="course__summery__item">
                                            <span class="sb_label">ভাষা </span><span
                                                class="sb_content">বাংলা</span>
                                        </div>
                                    </li>

                                    <li>
                                        <div class="course__summery__item">
                                            <span class="sb_label"> সার্টিফিকেট  </span><span class="sb_content">হ্যাঁ</span>
                                        </div>
                                    </li>

                                </ul>
                            </div>
                            <div class="course__summery__button">
                                <p>যোগাযোগ করুন</p>
                                <a class="default__button default__button--3" href="tel:+8801729277769"><i class="icofont-phone"></i> +৮৮০ ১৭২৯ ২৭ ৭৭ ৬৯</a>
                            </div>


                        </div>


                        <div class="blogsidebar__content__wraper__2" data-aos="fade-up">

                            <h4 class="sidebar__title">Follow Us</h4>
                            <div class="follow__icon">
                                <ul>
                                    <li>
                                        <a href="#"><i class="icofont-facebook"></i></a>
                                    </li>
                                    <li>
                                        <a href="#"><i class="icofont-youtube-play"></i></a>
                                    </li>
                                    <li>
                                        <a href="#"><i class="icofont-instagram"></i></a>
                                    </li>
                                </ul>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- footer__section__start -->
    <?php include ('include/footer.php');?>
    <!-- footer__section__end -->


</main>





<!-- JS here -->
<?php include ('include/js.php');?>


</body>

</html>